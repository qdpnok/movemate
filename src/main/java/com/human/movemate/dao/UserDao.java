...
public class UserDao {
    ...
    public Long save(User user) {
        ...
        @Language("SQL")
        String sql = """
        INSERT INTO USERS (name, user_id, password, email, phone_no)
        VALUES (?, ?, ?, ?, ?)
        """;
    ...
    // public -> 접근제한자. 해당 메서드를 이 클래스 외부에서도 호출할 수 있음을 의미
    // User -> 반환 타입. User 타입으로 반환. [return] 으로 반환하려는 값과 [같은 자료형]을 적어야 함.
    ...
    public User findById(String id) {
        ...
        List<User> list = jdbc.query(sql, new MemberRowMapper(), id);
    ...
    // public -> 접근제한자. 해당 메서드를 이 클래스 외부에서도 호출할 수 있음을 의미
    // boolean -> 반환 타입. boolean 타입으로 반환. [return] 으로 반환하려는 값과 [같은 자료형]을 적어야 함.
    // (Long id, User user) -> Long 타입의 id 매개변수, User 타입의 user 매개변수.
    //                             Long 타입의 값를 받아서 해당 매서드 내에서 id라는 이름으로 부름
    //                             User 타입의 값를 받아서 해당 매서드 내에서 member라는 이름으로 부름
    ...
    // public -> 접근제한자. 해당 메서드를 이 클래스 외부에서도 호출할 수 있음을 의미
    // boolean -> 반환 타입. boolean 타입으로 반환. [return] 으로 반환하려는 값과 [같은 자료형]을 적어야 함.
    // (Long id) -> Long 타입의 id 매개변수, User 타입의 member 매개변수.
    //              Long 타입의 값를 받아서 해당 매서드 내에서 id라는 이름으로 부름
    public boolean delete(Long no){
        // DELETE [테이블명] WHERE [조건]
        @Language("SQL")
        String sql = "DELETE USERS WHERE user_no = ?";
