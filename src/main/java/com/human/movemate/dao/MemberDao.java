...
public class MemberDao {
    ...
    public boolean save(Member member) {
        ...
        @Language("SQL")
        String sql = """
        INSERT INTO member (email, tel, pwd, name)
        VALUES (?, ?, ?, ?)
        """;
        ...
        return jdbc.update(sql, member.getEmail(), member.getTel(), member.getPwd(), member.getName()) > 0;
    }
    ...
    public Member findById(Long id) {
        ...
        List<Member> list = jdbc.query(sql, new MemberRowMapper());
    ...
    public boolean update(Long id, Member member){
        ...
        // jdbc.update는 삽입, 수정, 삭제에 성공한 행의 갯수를 반환함.
        // 때문에 결과값이 0보다 큰지 검사해서 성공/실패 여부를 확인
        return jdbc.update(sql, member.getPwd(), member.getName(), id) > 0;
    }
    ...
    static class MemberRowMapper implements RowMapper<Member> {
        ...
        public Member mapRow(ResultSet rs, int rowNum) throws SQLException {
            return new Member(
                    ...
                    // 오라클에서 날짜 타입은 timestamp 밖에 없기 때문에
                    // JAVA로 가져올 때 .toLocalDateTime() 메서드를 사용해 localDateTime 타입으로 변환을 해줘야함.
